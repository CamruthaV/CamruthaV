## Hi, I'm Amruthavalli Chivukula! 👋

I'm a cloud-focused engineer transitioning into **Cloud Security**, with hands-on experience in **Google Cloud Platform (GCP)** and a strong interest in building secure, well-architected systems.

## 🚀 About Me

- 🔭 I'm currently a Software Engineer at HSBC Technologies India.
- 💻 I love learning new things in the tech space, especially around Cloud, AI and Cybersecurity.
- 😄 Pronouns: She / Her

---

## ⚙️ Tech stack
Primarily focused on GCP, with working familiarity in AWS and Azure.

---

## ✅ Certifications
- Google Cloud Associate Cloud Engineer (GCP ACE)
- ISC2 Certified in Cybersecurity (CC)
- Microsoft Certified: Azure AI Fundamentals (AI-900)

---

## 🚀 Current Journey

- Preparing for **Microsoft Certified: Security, Compliance, and Identity Fundamentals (SC-900)**
- Building towards **Cloud Security specialization**
- Developing an **attacker mindset to better design defenses**

---

## 🛡️ What I’m Focused On

- Designing **secure cloud architectures**
- Implementing **least-privilege IAM**
- Understanding **common misconfigurations and attack paths**
- Strengthening **logging, monitoring, and auditability**

---

## ☁️ Cloud & Tools

- **GCP**: IAM, VPC, Cloud Storage, Logging, Compute Engine  
- **Security Concepts**: Least Privilege, Defense in Depth, Identity-based access  
- **Languages**: Python (basic scripting & automation)  

---

## 📂 Featured Work (In Progress)

Actively building hands-on projects in:

- Secure cloud architecture design  
- IAM misconfiguration analysis & fixes  
- Automation for cloud operations  

---

## 🧠 What I Care About

- Clarity over complexity  
- Security by design, not afterthought  
- Systems that are **observable, controlled, and resilient**  

---

## 📫 Let’s Connect

- Connect with me on [LinkedIn](https://linkedin.com/in/amruthavalli-chivukula)

Thanks for stopping by! Let's connect and explore the fascinating world of technology together. 🚀 

<!-- ![CamruthaV's Stats](https://github-readme-stats.vercel.app/api?username=CamruthaV&theme=vue-dark&show_icons=true&hide_border=true&count_private=true) -->

<!-- ## Also Learning
[![My Skills](https://skillicons.dev/icons?i=gcp&perline=1)](https://github.com/CamruthaV) </br>
[![My Skills](https://skillicons.dev/icons?i=aws,azure)](https://github.com/CamruthaV) -->

<!-- ## ⚒️ What I've been up to lately
   <h3>GSSoC'24 Extd.</h3>
  <div style='display:flex; align-items:center; gap: 50px;' align='center'>
<img src="https://raw.githubusercontent.com/GSSoC24/Postman-Challenge/main/docs/assets/Postman%20White.png" width="250px" height="250px" />
  <img src="https://raw.githubusercontent.com/GSSoC24/Postman-Challenge/main/docs/assets/1.png" width="250px" height="250px" />
  <img src="https://raw.githubusercontent.com/GSSoC24/Postman-Challenge/main/docs/assets/2.png" width="250px" height="250px" />
  <img src="https://raw.githubusercontent.com/GSSoC24/Postman-Challenge/main/docs/assets/3.png" width="250px" height="250px" />
  <img src="https://raw.githubusercontent.com/GSSoC24/Postman-Challenge/main/docs/assets/4.png" width="250px" height="250px" />
  <img src="https://raw.githubusercontent.com/GSSoC24/Postman-Challenge/main/docs/assets/5.png" width="250px" height="250px" />
  </div> -->

 <!-- <h3>Other Achievements</h3>
  <img src="![github-foundations](https://github.com/user-attachments/assets/bc24a96d-6439-41db-8bc5-ed19cab0b411)" /> -->
<!--
**CamruthaV/CamruthaV** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
